...
namespace DatingApp.API.Data
{
    public class AuthRepository : IAuthRepository
    {
        ...
        {
            var user = await _context.Users.FirstOrDefaultAsync(zz => zz.Username == username);
        ...
        private bool VerifyPasswordHash(string password, byte[] passwordHash, byte[] passwordSalt)
        {
            using(var hmac = new System.Security.Cryptography.HMACSHA1(passwordSalt)){
                ...
                {
                    if (computedHash[ii] != passwordHash[ii]) return false;
                }
            }
            return true;
        }
        ...
        private async Task<bool> UserExists(string username) {
            return await _context.Users.AnyAsync(zz => zz.Username == username);
        }
    }
}